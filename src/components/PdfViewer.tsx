...
declare global { interface Window { __pdfHandle?: PdfViewerHandle; } }

type FitMode = "off" | "width" | "page";

interface LatchedSelection {
  text: string; page: number; context: string;
}
...
export function PdfViewer({ tab }: { tab: PdfTab }) {
  const {
    updatePdfPage, updatePdfZoom, updatePdfLayout, updatePdfTotalPages,
    updatePdfPageRotation, addBookmark, bookmarks, settings, theme, updateRecentPosition,
  } = useAppStore();
  ...
  const [fitMode, setFitMode]     = useState<FitMode>(settings.autoFitPdf ? "width" : "off");
  ...
  const [searchHits, setSearchHits]   = useState<Array<{page:number; spanIdx:number; matchStart:number; matchLen:number}>>([]);
  ...
  const fileBookmarks = useMemo(
    () => bookmarks.filter(b => b.filePath === tab.filePath && b.selectedText && b.page != null),
    [bookmarks, tab.filePath],
  );

  useEffect(() => { setDarkInvert(settings.pdfDarkMode); }, [settings.pdfDarkMode]);
  useEffect(() => { setFitMode(settings.autoFitPdf ? "width" : "off"); }, [settings.autoFitPdf]);

  const cycleFitMode = () => setFitMode(m => m === "off" ? "width" : m === "width" ? "page" : "off");
  ...
  const computeScale = useCallback((
    ...
    if (fm !== "off" && cont) {
      ...
      if (fm === "width") {
        ...
        scale = Math.max(0.2, Math.min(Math.min(availW / vp0.width, availH / vp0.height), 6));
      }
    }
  ...
  const renderPage = useCallback(async (pageNum: number) => {
    ...
    if (existing) { try { existing.cancel(); } catch {} renderTasks.current.delete(pageNum); }
    ...
    try {
      ...
      const vpDevice  = page.getViewport({ scale: scale * dpr, rotation: extraRot });
      ...
      const renderTask = page.render({ canvasContext: ctx, viewport: vpDevice });
      ...
      pageStateRef.current.set(pageNum, { rendered: true, width: vpLogical.width, height: vpLogical.height });
  ...
  }, [computeScale]); // eslint-disable-line
  ...
  const evictPage = useCallback((pageNum: number) => {
    ...
    const wrapper = wrapperRefs.current.get(pageNum);
    if (wrapper) wrapper.querySelectorAll(".pdf-bm-hl").forEach(el => el.remove());
  ...
  const enqueue = useCallback((pages: number[]) => {
  ...
  const checkVisibility = useCallback(() => {
    ...
    wrapperRefs.current.forEach((wrapper, pageNum) => {
      ...
      if (inRender && !renderedPages.current.has(pageNum) && !activeRenders.current.has(pageNum)) {
        toRender.push(pageNum);
      }
      if (!inEvict && renderedPages.current.has(pageNum) && !activeRenders.current.has(pageNum)) {
        evictPage(pageNum);
      }
    });
    ...
    toRender.sort((a, b) => Math.abs(a - currentPage) - Math.abs(b - currentPage));
    ...
    (async () => {
      try {
        ...
        for (let i = 1; i <= pdfDoc.numPages; i++) {
          pageStateRef.current.set(i, { rendered: false, width: PLACEHOLDER_W, height: placeholderH });
        }
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      const pages: number[] = [];
      for (let i = startPage; i <= Math.min(startPage + 4, doc.numPages); i++) pages.push(i);
      for (let i = startPage - 1; i >= Math.max(1, startPage - 2); i--) pages.push(i);
      enqueue(pages);
    }, 30);
  ...
  useEffect(() => {
        ...
        for (const [, wrapper] of wrapperRefs.current) {
          ...
          if (r.top >= contRect.top - 10) {
            ...
            if (p !== tabRef.current.page) {
              ...
              window.dispatchEvent(new CustomEvent("pdf-page-change", { detail: { page: p } }));
            }
            break;
          }
        }
    ...
    return () => { cont.removeEventListener("scroll", h); cancelAnimationFrame(rafId); };
  }, [doc, tab.id, tab.filePath, updatePdfPage, updateRecentPosition, checkVisibility]);
  ...
  const goToPage = useCallback((n: number) => {
    ...
    updatePdfPage(tab.id, p);
    const el = containerRef.current?.querySelector(`[data-page="${p}"]`) as HTMLElement | null;
  ...
  useEffect(() => {
    ...
    return () => { delete window.__pdfHandle; };
  }, [toc, goToPage]);
  ...
  const paintHighlights = useCallback((
    ...
    const spans = Array.from(textDiv.querySelectorAll("span[role=presentation], span:not(.pdf-search-mark)")) as HTMLElement[];
    if (spans.length === 0) {
      // fallback: all spans
      const allSpans = Array.from(textDiv.querySelectorAll("span")) as HTMLElement[];
    ...
    let flatText = "";
    const spanMeta: {
      start: number; end: number; el: HTMLElement;
    ...
    for (const bm of pageBms) {
      ...
      const hitSpans = spanMeta.filter(s => s.end > flatStart && s.start < flatEnd);
  ...
  useEffect(() => {
    const onPointerUp = () => {
      ...
      requestAnimationFrame(() => {
        ...
        ).map(s => s.textContent ?? "").join("").slice(0, 300);
  ...
  const navigateBookmark = useCallback((delta: 1 | -1) => {
    if (fileBookmarks.length === 0) return;
    const next = (bmNavIdx + delta + fileBookmarks.length) % fileBookmarks.length;
    setBmNavIdx(next);
    ...
    setTimeout(() => {
      ...
      for (const hl of hls) {
        if (hl.dataset.note === (bm.message ?? "") || hls.length === 1) {
          ...
          setTimeout(() => { hl.style.outline = ""; }, 900);
          break;
        }
      }
    }, 350);
  ...
  const doSearch = useCallback(async (q: string) => {
    const d = docRef.current;
    if (!d || !q.trim()) { setSearchHits([]); return; }
    ...
    const hits: Array<{page:number; spanIdx:number; matchStart:number; matchLen:number}> = [];
    ...
    for (let pn = 1; pn <= d.numPages; pn++) {
      ...
      for (let i = 0; i < spans.length; i++) {
        ...
        spanMap.push({ start: flat.length, end: flat.length + t.length, idx: i });
        flat += t;
      }
      ...
      while (pos !== -1) {
        ...
        const overlapping = spanMap.filter(s => s.end > pos && s.start < matchEnd);
        for (const hit of overlapping) {
          hits.push({ page: pn, spanIdx: hit.idx, matchStart: pos - hit.start, matchLen: lq.length });
        }
  ...
  const applySearchHighlight = (hits: typeof searchHits, pos: number) => {
    ...
    for (let i = 0; i < hits.length; i++) {
      ...
      span.textContent = "";
      if (start > 0) span.appendChild(document.createTextNode(text.slice(0, start)));
      const mark = document.createElement("mark");
      mark.className = "pdf-search-mark" + (i === pos ? " current" : "");
      mark.textContent = text.slice(start, end);
      span.appendChild(mark);
      if (end < text.length) span.appendChild(document.createTextNode(text.slice(end)));
    }
  };

  const moveSearch = useCallback((delta: 1 | -1) => {
  ...
  }, [searchHits, searchPos, goToPage]); // eslint-disable-line
  ...
  useEffect(() => {
    const h = (e: KeyboardEvent) => {
      ...
      if ((e.metaKey || e.ctrlKey) && e.key === "f") {
        e.preventDefault();
        setShowSearch(v => { if (!v) setTimeout(() => searchRef.current?.focus(), 30); return true; });
        return;
      }
      if (e.metaKey || e.ctrlKey || e.altKey) return;

      if (showSearch && inInput) {
        if (e.key === "Escape") { e.preventDefault(); setShowSearch(false); setSearchQuery(""); clearSearch(); }
      ...
      switch (e.key) {
        case "j": case "ArrowDown": cont?.scrollBy({ top: 80,  behavior: "smooth" }); e.preventDefault(); break;
        case "k": case "ArrowUp":   cont?.scrollBy({ top: -80, behavior: "smooth" }); e.preventDefault(); break;
        // h/l — horizontal scroll for when page overflows the x-axis at high zoom
        case "h": case "ArrowLeft":  cont?.scrollBy({ left: -80, behavior: "smooth" }); e.preventDefault(); break;
        case "l": case "ArrowRight": cont?.scrollBy({ left:  80, behavior: "smooth" }); e.preventDefault(); break;
        case "d": cont?.scrollBy({ top:  window.innerHeight / 2, behavior: "smooth" }); e.preventDefault(); break;
        case "u": cont?.scrollBy({ top: -window.innerHeight / 2, behavior: "smooth" }); e.preventDefault(); break;
        case " ": cont?.scrollBy({ top:  window.innerHeight, behavior: "smooth" }); e.preventDefault(); break;
        case "n": if (d) { goToPage(tab.page + 1); e.preventDefault(); } break;
        case "N": if (d) { goToPage(tab.page - 1); e.preventDefault(); } break;
        case "g": goToPage(1); e.preventDefault(); break;
        case "G": if (d) { goToPage(d.numPages); e.preventDefault(); } break;
        case "+": case "=": updatePdfZoom(tab.id, Math.min(tab.zoom + 0.2, 5)); e.preventDefault(); break;
        case "-": updatePdfZoom(tab.id, Math.max(tab.zoom - 0.2, 0.2)); e.preventDefault(); break;
        case "0": updatePdfZoom(tab.id, 1); e.preventDefault(); break;
        case "a": cycleFitMode(); e.preventDefault(); break;
        case "p": setShowJump(true); e.preventDefault(); break;
        case "/": setShowSearch(v => { if (!v) setTimeout(() => searchRef.current?.focus(), 30); return true; }); e.preventDefault(); break;
        // "b" = open bookmark list (add / local / global) via global QuickModal
        case "b": window.dispatchEvent(new CustomEvent("folio:open-bookmarks")); e.preventDefault(); break;
        // "m" = open add-bookmark modal with current selection
        case "m": setShowBmAdd(true); e.preventDefault(); break;
        case "]": navigateBookmark(1); e.preventDefault(); break;
        case "[": navigateBookmark(-1); e.preventDefault(); break;
        case "i": setDarkInvert(v => !v); e.preventDefault(); break;
        case "Escape": setShowSearch(false); setShowJump(false); clearSearch(); break;
      }
    };
  ...
  if (error) return (
    <div className="pdf-error"><span>⚠ {error}</span></div>
  );
  if (!doc) return null;

  const pageNums   = Array.from({ length: doc.numPages }, (_, i) => i + 1);
  const pageGroups = tab.layout === "double"
    ? pageNums.reduce<number[][]>((a, n, i) => { if (i%2===0) a.push([n]); else a[a.length-1].push(n); return a; }, [])
    : pageNums.map(n => [n]);
  ...
  return (
    <div className="pdf-wrapper">
      ...
      <div className="pdf-toolbar">
        <button className="pdf-tool-btn" onClick={() => goToPage(tab.page - 1)} disabled={tab.page <= 1} title="Prev (N)">‹</button>
        <span className="pdf-page-info">
          <input className="pdf-page-input" type="number" value={tab.page} min={1} max={doc.numPages}
            onChange={e => goToPage(parseInt(e.target.value) || 1)}
            onFocus={e => e.target.select()} />
          <span className="pdf-page-sep">/ {doc.numPages}</span>
        </span>
        <button className="pdf-tool-btn" onClick={() => goToPage(tab.page + 1)} disabled={tab.page >= doc.numPages} title="Next (n)">›</button>
        <div className="pdf-toolbar-sep" />
        <button className="pdf-tool-btn" onClick={() => updatePdfZoom(tab.id, Math.max(tab.zoom - 0.2, 0.2))} title="Zoom out (-)">−</button>
        <span className="pdf-zoom-label">{Math.round(tab.zoom * 100)}%</span>
        <button className="pdf-tool-btn" onClick={() => updatePdfZoom(tab.id, Math.min(tab.zoom + 0.2, 5))} title="Zoom in (+)">+</button>
        <button className="pdf-tool-btn" onClick={() => updatePdfZoom(tab.id, 1)} title="Reset (0)">⊡</button>
        <button
          className={`pdf-tool-btn ${fitMode !== "off" ? "active" : ""}`}
          onClick={cycleFitMode}
          title={`Auto-fit: ${fitMode} — click to cycle (a)`}
        >{fitMode === "width" ? "⟺" : fitMode === "page" ? "⊠" : "⟷"}</button>
        {fitMode !== "off" && <span className="pdf-fit-label">{fitMode}</span>}
        <div className="pdf-toolbar-sep" />
        <button className={`pdf-tool-btn ${tab.layout === "single" ? "active" : ""}`} onClick={() => updatePdfLayout(tab.id, "single")} title="Single page">▬</button>
        <button className={`pdf-tool-btn ${tab.layout === "double" ? "active" : ""}`} onClick={() => updatePdfLayout(tab.id, "double")} title="Double page">▬▬</button>
        <div className="pdf-toolbar-sep" />
        <button className="pdf-tool-btn" onClick={() => updatePdfPageRotation(tab.id, tab.page, 90)} title="Rotate CW">↻</button>
        <button className="pdf-tool-btn" onClick={() => updatePdfPageRotation(tab.id, tab.page, -90)} title="Rotate CCW">↺</button>
        <div className="pdf-toolbar-sep" />
        <button className={`pdf-tool-btn ${shouldInvert ? "active" : ""}`} onClick={() => setDarkInvert(v => !v)} title="Dark invert (i)">◑</button>
        <div className="pdf-toolbar-sep" />
        <button className="pdf-tool-btn" onClick={() => { setShowSearch(true); setTimeout(() => searchRef.current?.focus(), 30); }} title="Search (/ or Ctrl+F)">🔍</button>
        <button className="pdf-tool-btn" onClick={() => setShowJump(true)} title="Jump (p)">⌕</button>
        <button className="pdf-tool-btn" onClick={() => setShowBmAdd(true)} title="Bookmark selection (m)">⊕</button>
      </div>
      ...
      {showSearch && (
        <div className="pdf-search-bar">
          ...
          <input ref={searchRef} className="pdf-search-input"
            placeholder="Search in PDF…" value={searchQuery}
            onChange={e => { setSearchQuery(e.target.value); doSearch(e.target.value); }}
            onKeyDown={e => {
              if (e.key === "Escape") { setShowSearch(false); setSearchQuery(""); clearSearch(); }
              if (e.key === "Enter") moveSearch(e.shiftKey ? -1 : 1);
            }} autoFocus />
          {searchHits.length > 0 && <span className="pdf-search-count">{searchPos+1}/{searchHits.length}</span>}
          {searchQuery && searchHits.length === 0 && <span className="pdf-search-count pdf-search-none">No matches</span>}
          <button className="pdf-search-nav" onClick={() => moveSearch(-1)}>↑</button>
          <button className="pdf-search-nav" onClick={() => moveSearch(1)}>↓</button>
          <button className="pdf-search-close" onClick={() => { setShowSearch(false); setSearchQuery(""); clearSearch(); }}>×</button>
        </div>
      )}
            ...
            {group.map(pageNum => {
              ...
              return (
                <div
                  key={pageNum}
                  data-page={pageNum}
                  ref={el => { if (el) wrapperRefs.current.set(pageNum, el as HTMLDivElement); else wrapperRefs.current.delete(pageNum); }}
                  className="pdf-page-wrapper"
                  style={{
                    width: `${w}px`,
                    height: `${h}px`,
                    filter: shouldInvert ? "invert(0.88) sepia(0.2) hue-rotate(180deg)" : undefined,
                  }}
                >
                  <canvas
                  ...
                  <div
                    ...
                    ref={el => { if (el) textLayerRefs.current.set(pageNum, el as HTMLDivElement); else textLayerRefs.current.delete(pageNum); }}
                  />
      ...
      {tooltip && tooltip.note && (
        <div
          ...
          style={{ left: tooltip.x + 14, top: tooltip.y - 8, position: "fixed", pointerEvents: "none", zIndex: 9999 }}
        >
          {tooltip.note}
        </div>
      )}
      ...
      {showJump && (
        <PdfJumpModal
          ...
          currentPage={tab.page} totalPages={doc.numPages}
          onGoPage={n => { goToPage(n); setShowJump(false); }}
        ...
        <BookmarkModal
          ...
          onSave={(msg, color, scope) => {
            addBookmark({
              ...
              fileType: "pdf", scope: scope ?? "local",
              page: latchedSel?.page ?? tab.page,
              selectedText: latchedSel?.text,
              textContext: latchedSel?.context,
              highlightColor: color, message: msg,
              preview: latchedSel?.text
                ? `"${latchedSel.text.slice(0, 60)}"`
                : `Page ${tab.page}`,
              pageHeading: toc.filter(t => t.page <= (latchedSel?.page ?? tab.page)).slice(-1)[0]?.title,
            });
...
async function flattenOutline(
  doc: PDFDocumentProxy, items: any[], level: number,
): Promise<TocItem[]> {
    ...
    try {
      if (item.dest) {
        const dest = typeof item.dest === "string"
          ...
          : item.dest;
        if (dest) { const idx = await doc.getPageIndex(dest[0]); page = idx + 1; }
      }
    } catch {}
    result.push({ title: item.title ?? "(untitled)", page, level });
    if (item.items?.length) result.push(...await flattenOutline(doc, item.items, level + 1));
  }
  return result;
}
